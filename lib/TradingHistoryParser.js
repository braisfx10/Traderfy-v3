class TradingHistoryParser {
  ...
  parseHTML(htmlContent) {
    try {
      console.log('TradingHistoryParser: Starting parseHTML');
      
      // Use JSDOM for Node.js environment or DOMParser for browser
      let doc;
      if (typeof window !== 'undefined' && window.DOMParser) {
        // Browser environment
        console.log('TradingHistoryParser: Using browser DOMParser');
        const parser = new DOMParser();
        doc = parser.parseFromString(htmlContent, 'text/html');
      } else {
        // Node.js environment
        console.log('TradingHistoryParser: Using JSDOM for Node.js');
        const { JSDOM } = require('jsdom');
        const dom = new JSDOM(htmlContent);
        doc = dom.window.document;
      }
      
      console.log('TradingHistoryParser: Document parsed, detecting platform');
      
      // Detect platform
      const platform = this.detectPlatform(doc, htmlContent);
      console.log('TradingHistoryParser: Platform detected:', platform);
      
      if (platform === this.platforms.MT5) {
        console.log('TradingHistoryParser: Parsing as MT5');
        return this.parseMT5(doc);
      } else if (platform === this.platforms.CTRADER) {
        console.log('TradingHistoryParser: Parsing as cTrader');
        return this.parseCTrader(doc);
      } else {
        throw new Error('Plataforma no reconocida. Solo se soportan MetaTrader 5 y cTrader.');
      }
    } catch (error) {
      console.error('TradingHistoryParser: Error in parseHTML:', error);
