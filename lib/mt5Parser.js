...
export class MT5Parser {
  ...
  extractPositions() {
    ...
    this.$('tr').each((index, row) => {
      ...
      // Detectar fin de sección (cuando llega a tabla vacía o nueva sección)
      ...
      if (inPositionsSection) {
        ...
        // Buscar fila de encabezados (bgcolor="#E5F0FC" o que contenga "Fecha/Hora")
        if (!headerRowFound) {
        ...
        // Procesar filas de datos - deben tener bgcolor="#FFFFFF" o "#F7F7F7" y 13+ celdas
        if (headerRowFound && cells.length >= 13) {
          const bgColor = this.$(row).attr('bgcolor')
          if (bgColor === '#FFFFFF' || bgColor === '#F7F7F7') {
            const position = this.parsePositionRow(cells)
            if (position && position.symbol && position.profit !== null) {
              positions.push(position)
              console.log('Parsed position:', position.symbol, position.type, position.profit)
            }
          }
        }
      }
  ...
  parsePositionRow(cells) {
    try {
      ...
      console.log('DEBUG: Raw position cell texts:', cellTexts)

      // Estructura real de MT5 Posiciones (13 columnas):
      ...
      if (!cellTexts[2] || !cellTexts[3] || !cellTexts[12]) {
      ...
      const position = {
        ...
        ticket: cellTexts[1],
        symbol: cellTexts[2],
        type: cellTexts[3] ? cellTexts[3].toLowerCase() : 'buy',
      ...
      console.log('DEBUG: Parsed position object:', position)
      return position
    } catch (error) {
  ...
  extractResults() {
    ...
    this.$('tr').each((index, row) => {
      ...
      if (inResultsSection) {
        ...
        if (cells.length >= 2) {
          ...
          console.log('Processing results row:', cellTexts)
          
          // Mapear los valores específicos según el HTML real
          for (let i = 0; i < cellTexts.length - 1; i += 2) {
            ...
            if (label && label.includes('Posiciones largas')) {
              ...
              if (match) {
                ...
                console.log('Long positions found:', results.long_positions)
              }
            }
          }
        }
    ...
    console.log('Final results extracted:', results)
    return results
  }
  ...
  normalizeDateTime(dateStr) {
    ...
    // Convertir 2025.08.19 16:01:32 a formato JavaScript Date compatible
    const match = dateStr.match(/(\d{4})\.(\d{2})\.(\d{2})\s+(\d{2}):(\d{2}):(\d{2})/)
    if (match) {
      // Formato ISO: YYYY-MM-DDTHH:MM:SS (compatible con JavaScript Date)
  ...
  parseNumber(str) {
    ...
    // Remover espacios, mantener puntos y guiones
    // "1 526.20" -> "1526.20"
    // "-662.35" -> "-662.35"
    ...
    console.log(`parseNumber: "${str}" -> "${cleaned}" -> ${number}`)
