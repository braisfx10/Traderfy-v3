...
export const getUserLabels = async (userId) => {
  ...
  if (!supabase) {
    try {
      const demoLabels = JSON.parse(localStorage.getItem('demo_labels') || '[]')
      return demoLabels.filter(label => label.user_id === (userId || 'demo'))
    } catch (error) {
      console.error('Error loading demo labels:', error)
      return []
    }
  }
...
export const updateLabel = async (labelId, updates) => {
  try {
    if (!supabase) {
      ...
      const labelsData = JSON.parse(localStorage.getItem('demo_labels') || '[]')
      const labelIndex = labelsData.findIndex(label => label.id === labelId)
      ...
      labelsData[labelIndex] = updatedLabel
      localStorage.setItem('demo_labels', JSON.stringify(labelsData))
...
export const deleteLabel = async (labelId) => {
  try {
    if (!supabase) {
      // Modo demo - usar localStorage
      const labelsData = JSON.parse(localStorage.getItem('demo_labels') || '[]')
      const filteredLabels = labelsData.filter(label => label.id !== labelId)
      localStorage.setItem('demo_labels', JSON.stringify(filteredLabels))
      
      // También remover la etiqueta de todas las cuentas
      const accountsData = JSON.parse(localStorage.getItem('demo_accounts') || '[]')
      const updatedAccounts = accountsData.map(account => ({
        ...account,
        labels: (account.labels || []).filter(label => label.id !== labelId)
      }))
...
export const DEFAULT_LABELS = [
  ...
  { name: 'Challenge', color: '#6B7280', system: true }
]
