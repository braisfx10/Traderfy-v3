...
export class UnifiedTradingParser {
  ...
  parseMT5TradeRow(cells) {
    try {
      ...
      // Filtrar transacciones que no son trades (depósitos, retiradas, etc.)
      const symbol = cellTexts[2] ? cellTexts[2].trim() : ''
      const nonTradingSymbols = ['Depósito', 'Retirada', 'Deposit', 'Withdrawal', 'Balance', 'Credit', 'Total Neto', 'Initial Deposit']
      if (nonTradingSymbols.some(term => symbol.includes(term) || symbol === term)) {
        console.log('Filtering out non-trading MT5 transaction:', symbol)
        return null
      }
  ...
  extractCTraderTrades() {
    ...
    let headerFound = false
    let historialTableProcessed = false // Flag para procesar solo la primera tabla

    console.log('Searching for cTrader Historial section...')
    console.log('Total tables found:', this.$('table').length)

    this.$('table').each((tableIndex, table) => {
      // Si ya procesamos una tabla con historial, saltar las demás
      if (historialTableProcessed) {
        console.log(`Skipping table ${tableIndex} - Historial already processed`)
        return
      }
      
      console.log(`Processing table ${tableIndex}...`)
      let rowsInThisTable = 0
      let foundHistorialInThisTable = false
      
      this.$(table).find('tr').each((rowIndex, row) => {
        const rowText = this.$(row).text().trim()
        
        // Detectar sección Historial
        if (rowText.includes('Historial') && !rowText.includes('Posiciones')) {
          inHistorialSection = true
          headerFound = false
          foundHistorialInThisTable = true
          console.log(`Found cTrader Historial section in table ${tableIndex}`)
          return
        }
        
        // Detectar fin de sección (cuando encuentra "Posiciones" o sección summary)
        if (inHistorialSection && (rowText.includes('Posiciones') || rowText.includes('Sin Posiciones'))) {
          inHistorialSection = false
          if (foundHistorialInThisTable) {
            historialTableProcessed = true // Marcar como procesado
          }
          console.log(`Found end of Historial section at Posiciones in table ${tableIndex}`)
          return
        }
        ...
        if (inHistorialSection) {
          ...
          if (!headerFound && rowText.includes('Símbolo') && (rowText.includes('USD neto') || rowText.includes('Dirección de apertura'))) {
            headerFound = true
            console.log(`Found cTrader header row in table ${tableIndex}`)
            return
          }
          ...
          if (headerFound && cells.length >= 8) {
            ...
            if (hasValidData) {
              ...
              if (trade) {
                trades.push(trade)
                rowsInThisTable++
                console.log(`Parsed cTrader trade ${trades.length}: ${trade.symbol} ${trade.type} ${trade.profit}`)
              }
            }
          }
        }
      ...
      console.log(`Table ${tableIndex} contributed ${rowsInThisTable} trades`)
      
      // Si esta tabla contribuyó trades, marcarla como procesada
      if (rowsInThisTable > 0) {
        historialTableProcessed = true
      }
    })
  ...
  parseCTraderTradeRow(cells) {
    try {
      ...
      // Verificar que no sea fila vacía, de totales, o transacciones no-trading
      if (cellTexts.every(text => text === '') || 
          cellTexts.some(text => text.includes('Totales')) ||
          cellTexts.some(text => text.includes('Total Neto')) ||
          !cellTexts[1] || cellTexts[1].length < 2) {
        return null
      }

      // Filtrar transacciones que no son trades (depósitos, retiradas, etc.)
      const symbol = cellTexts[1].trim()
      const nonTradingSymbols = ['Depósito', 'Retirada', 'Deposit', 'Withdrawal', 'Balance', 'Credit', 'Total Neto', 'Initial Deposit']
      if (nonTradingSymbols.some(term => symbol.includes(term) || symbol === term)) {
        console.log('Filtering out non-trading transaction:', symbol)
        return null
      }
