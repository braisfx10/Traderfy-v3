  ...
  extractCTraderTrades() {
    ...
    this.$('table').each((tableIndex, table) => {
      this.$(table).find('tr').each((rowIndex, row) => {
        ...
        if (rowText.includes('Historial') && !rowText.includes('Extracto')) {
        ...
        // Detectar fin de sección
        if (inHistorialSection && rowText.includes('Totales')) {
        ...
        if (inHistorialSection) {
          ...
          // Buscar encabezados
          if (!headerFound && rowText.includes('Símbolo') && rowText.includes('USD neto')) {
            headerFound = true
            return
          }
  ...
  parseCTraderTradeRow(cells) {
    try {
      ...
      return {
        ...
        symbol: cellTexts[1],
        type: (cellTexts[2] || '').toLowerCase(),
        volume: this.extractVolumeFromCTrader(cellTexts[6]), // "2.00 Lotes" -> 2.00
