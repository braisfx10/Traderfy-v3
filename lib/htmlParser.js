...
export class TradingReportParser {
  ...
  convertUnifiedResult(unifiedResult, reportType) {
    ...
    const summary = {
      ...
      deposit: 25000, // Default, se puede mejorar extrayendo del HTML
      initialBalance: 25000,
      balance: 25000 + (unifiedResult.results.net_profit || 0),
...
export const parseHTMLReport = async (htmlContent) => {
  try {
    console.log('parseHTMLReport: Starting to parse HTML content of length:', htmlContent.length)
    
    // Usar la nueva TradingHistoryParser
    const { default: TradingHistoryParser } = await import('./TradingHistoryParser.js')
    console.log('parseHTMLReport: TradingHistoryParser imported successfully')
    
    const parser = new TradingHistoryParser()
    const tradingData = parser.parseHTML(htmlContent)
    console.log('parseHTMLReport: Parsing completed, platform:', tradingData.platform)

    // Convertir al formato esperado por la aplicación
    const convertedResult = convertTradingDataToLegacyFormat(tradingData)
    console.log('parseHTMLReport: Conversion completed, final trades count:', convertedResult.trades.length)
    
    return convertedResult
  } catch (error) {
    console.error('Error using TradingHistoryParser, falling back to old parser:', error)
...
// Función para convertir el nuevo formato al formato legacy esperado por la aplicación
function convertTradingDataToLegacyFormat(tradingData) {
  let trades = []
  
  // Convertir trades según la plataforma
  if (tradingData.platform === 'MetaTrader5') {
    trades = tradingData.positions.map((position, index) => ({
      symbol: position.symbol || '',
      direction: capitalizeFirst(position.type || 'buy'),
      close_time: position.closeTime || position.openTime || '',
      entry_price: position.openPrice || 0,
      close_price: position.closePrice || 0,
      lots: position.volume || 0,
      pnl: position.profit || 0,
      position_id: position.ticket || `trade-${index + 1}`,
      commission: position.commission || 0,
      swap: position.swap || 0,
      stop_loss: position.sl || 0,
      take_profit: position.tp || 0,
      id: `mt5-${index + 1}`,
      account_id: tradingData.accountInfo?.account || 'mt5-account',
      open_time: position.openTime || '',
      user_id: 'demo'
    }))
  } else if (tradingData.platform === 'cTrader') {
    trades = tradingData.history.map((trade, index) => ({
      symbol: trade.symbol || '',
      direction: capitalizeFirst(trade.direction || 'buy'),
      close_time: trade.closeTime || '',
      entry_price: trade.entryPrice || 0,
      close_price: trade.exitPrice || 0,
      lots: parseFloat(trade.volume?.replace(' Lotes', '')) || 0,
      pnl: trade.netUSD || 0,
      position_id: `trade-${index + 1}`,
      commission: 0,
      swap: 0,
      ...
      take_profit: 0,
      id: `ctrader-${index + 1}`,
      account_id: tradingData.accountInfo?.account || 'ctrader-account',
      open_time: trade.closeTime || '',
      user_id: 'demo'
    }))
  }
  ...
  // Extraer información de la cuenta
  const accountInfo = {
    accountNumber: tradingData.accountInfo?.account || '',
    currency: tradingData.accountInfo?.currency || 'USD',
    company: tradingData.accountInfo?.company || '',
    reportDate: tradingData.accountInfo?.date || ''
  }

  // Usar las métricas calculadas por la nueva clase
  const metrics = tradingData.metrics || {}
  
  // Crear resumen usando las métricas avanzadas
  const summary = {
    netProfit: metrics.netProfit || 0,
    grossProfit: metrics.totalProfit || 0,
    grossLoss: metrics.totalLoss || 0,
    profitFactor: metrics.profitFactor || 0,
    totalTrades: metrics.totalTrades || trades.length,
    longPositions: trades.filter(t => t.direction === 'Buy').length,
    shortPositions: trades.filter(t => t.direction === 'Sell').length,
    bestTrade: metrics.largestWin || 0,
    worstTrade: -metrics.largestLoss || 0,
    ...
    deposit: tradingData.summary?.deposit || 15000,
    withdrawal: tradingData.summary?.withdrawal || 0,
    initialBalance: tradingData.summary?.deposit || 15000,
    balance: tradingData.summary?.balance || 15000,
    realizedPnl: metrics.netProfit || 0
  }

  console.log('Converted trading data:', {
  ...
  return {
    ...
    accountInfo,
    summary,
    reportType: tradingData.platform,
    message: `Datos procesados correctamente (${tradingData.platform})`
  }
}
