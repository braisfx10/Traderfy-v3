...
export const parseHTMLReport = async (htmlContent) => {
  try {
    ...
    // Usar el UnifiedTradingParser en su lugar
    const { UnifiedTradingParser } = await import('./unifiedTradingParser.js')
    console.log('parseHTMLReport: UnifiedTradingParser imported successfully')
    
    const unifiedParser = new UnifiedTradingParser(htmlContent)
    console.log('parseHTMLReport: UnifiedTradingParser created, detected type:', unifiedParser.reportType)
    
    const unifiedResult = unifiedParser.parse()
    console.log('parseHTMLReport: Parsing completed, trades found:', unifiedResult.trades.length)
    
    // Convertir el resultado al formato esperado
    const parser = new TradingReportParser(htmlContent)
    const convertedResult = parser.convertUnifiedResult(unifiedResult, unifiedParser.reportType)
    console.log('parseHTMLReport: Conversion completed, final trades count:', convertedResult.trades.length)
    
    return convertedResult
  } catch (error) {
    console.error('Error using UnifiedParser, falling back to old parser:', error)
