...
export const parseHTMLReport = async (htmlContent) => {
  try {
    ...
    // Validar que tenemos contenido HTML
    ...
    console.log('parseHTMLReport: Base parsing completed, trades found:', unifiedResult.trades?.length || 0)
    console.log('parseHTMLReport: Report type:', unifiedParser.reportType)
    
    // Validar que tenemos resultados
    if (!unifiedResult || !unifiedResult.trades) {
      console.error('parseHTMLReport: No se pudieron extraer trades del archivo')
      return {
        trades: [],
        accountInfo: {},
        summary: {},
        reportType: null,
        message: "No se pudieron encontrar operaciones en el archivo HTML"
      }
    }
    ...
    console.log('parseHTMLReport: Enhanced conversion completed, final trades count:', convertedResult.trades?.length || 0)
    
    // Validar el resultado final
    if (!convertedResult || !convertedResult.trades) {
      console.error('parseHTMLReport: Error en la conversión final')
      return {
        trades: [],
        accountInfo: {},
        summary: {},
    ...
    console.error('Error using Enhanced parser:', error)
    console.error('Error stack:', error.stack)
    
    try {
      // Fallback al parser viejo en caso de error
      console.log('parseHTMLReport: Attempting fallback to old parser')
      const parser = new TradingReportParser(htmlContent)
      const fallbackResult = parser.parse()
      
      if (fallbackResult && fallbackResult.trades) {
        console.log('parseHTMLReport: Fallback successful, trades:', fallbackResult.trades.length)
        return fallbackResult
      }
    } catch (fallbackError) {
      console.error('parseHTMLReport: Fallback parser also failed:', fallbackError)
    }
